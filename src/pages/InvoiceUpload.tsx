...
export const InvoiceUpload = () => {
  ...
  return (
    <div className="max-w-4xl mx-auto">
      {!invoice ? (
        <div className="bg-white p-6 sm:p-12 rounded-3xl shadow-sm border-2 border-dashed border-slate-200 text-center flex flex-col items-center">
          <div className="w-16 h-16 bg-emerald-100 text-emerald-600 rounded-full flex items-center justify-center mb-6">
            <UploadCloud size={32} />
          </div>
          <h2 className="text-xl sm:text-2xl font-bold text-slate-800 mb-2">Novo Documento</h2>
          <p className="text-slate-500 mb-8 text-sm sm:text-base">Arraste a sua fatura ou selecione um ficheiro</p>
          <input type="file" accept="image/jpeg,image/png,image/webp,image/bmp,application/pdf" onChange={e => setFile(e.target.files?.[0] || null)} className="mb-6 block w-full max-w-xs mx-auto text-sm text-slate-500" />
          <button
            disabled={!file || loading}
            onClick={handleProcess}
            className="w-full sm:w-auto bg-slate-900 text-white px-8 py-3 rounded-xl font-bold hover:bg-black disabled:opacity-50 flex items-center justify-center gap-2"
          >
        ...
        <div className="bg-white rounded-3xl shadow-sm overflow-hidden border border-slate-100">
          <div className="p-4 sm:p-6 bg-slate-50 border-b flex justify-between items-center gap-3">
            <h3 className="font-bold text-slate-800">Confirmar Dados Extraídos</h3>
            <button onClick={() => setInvoice(null)} className="text-red-500 text-sm shrink-0">Cancelar</button>
          </div>
            ...
            <div className="px-4 sm:px-8 pb-8">
              ...
              <div className="mt-3 divide-y divide-slate-100 border rounded-xl overflow-hidden">
                {invoice.items.map((item: any, idx: number) => (
                  <div key={idx} className="flex items-center gap-2 sm:gap-3 px-3 sm:px-4 py-2.5">
                    <input
                      className="flex-1 min-w-0 outline-none text-sm text-slate-700 bg-transparent"
                    ...
                    <input
                      type="number"
                      step="0.01"
                      className="w-16 sm:w-24 shrink-0 outline-none text-sm text-right font-semibold text-slate-700 bg-transparent"
          ...
          <div className="p-4 sm:p-8 border-t bg-slate-50 flex justify-end">
            <button
              onClick={saveInvoice}
              disabled={saving}
              className="w-full sm:w-auto bg-emerald-600 text-white px-10 py-3 rounded-xl font-bold hover:bg-emerald-700 shadow-lg shadow-emerald-200 disabled:opacity-50 flex items-center justify-center gap-2"
            >
